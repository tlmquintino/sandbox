...
public:
  ...
  bool wait_for_and_pop( T& popped_item, std::chrono::microseconds t )
  {
    std::unique_lock<std::mutex> lock(m_); // note: unique_lock is needed for std::condition_variable::wait_for
    if( queue_.empty() )
    {
        if( data_cond_.wait_for(lock,t) == std::cv_status::timeout )
            return false;
    }
    popped_item = queue_.front();
    queue_.pop();
